...
handleClick(direction, unit) {
  ...
  if (direction.indexOf("up") !== -1) { // up button was clicked
    if (unit.indexOf("mph") !== -1) { // increment mph
      ...
      axios
        .get("http://treadmill-helper-treadmill-helper.6923.rh-us-east-1.openshiftapps.com/?mph="+ newMph)
        .then(response => {
      ...
      axios
        .get("http://treadmill-helper-treadmill-helper.6923.rh-us-east-1.openshiftapps.com/?kph="+ newKph)
        .then(response => {
          console.log(response.data);
          this.setState({
